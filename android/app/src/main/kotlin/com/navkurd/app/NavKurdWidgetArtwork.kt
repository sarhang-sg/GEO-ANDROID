...
object NavKurdWidgetArtwork {
    ...
    fun weatherIcon(kind: String, isDay: Boolean, frame: Long): Bitmap {
        ...
        when (kind) {
            ...
            "PARTLY_CLOUDY" -> {
                if (isDay) drawSun(canvas, 38f, 37f, 17f, paint) else drawMoon(canvas, 37f, 35f, 18f, paint)
                drawCloud(canvas, 70f, 71f, false, paint, 0.78f)
            }
            "CLOUDY" -> {
                drawCloud(canvas, 64f, 64f, true, paint, 0.92f)
            }
            "FOG" -> {
    ...
    private fun palette(kind: String, isDay: Boolean, phase: String, season: String): Pair<Int, Int> {
        ...
        return when {
            !isDay || phase == "night" || phase == "late_night" -> Color.rgb(9, 18, 50) to Color.rgb(34, 22, 72)
            phase == "pre_dawn" -> Color.rgb(70, 76, 135) to Color.rgb(222, 126, 101)
    ...
    private fun drawSeasonAccent(
        canvas: Canvas,
        season: String,
        isDay: Boolean,
        frame: Long,
    ) {
        val random = Random(1200L + frame)
        val paint = Paint(Paint.ANTI_ALIAS_FLAG)
        val count = if (isDay) 15 else 9
        repeat(count) { index ->
            val x = 12f + random.nextFloat() * (SCENE_WIDTH - 24f)
            val y = 18f + random.nextFloat() * 155f
            val radius = 1.2f + random.nextFloat() * 2.5f
            when (season) {
                "spring" -> {
                    paint.color = Color.argb(80 + random.nextInt(75), 255, 187 + index % 40, 220)
                    canvas.drawCircle(x - radius, y, radius, paint)
                    canvas.drawCircle(x + radius, y, radius, paint)
                }
                "summer" -> {
                    paint.color = Color.argb(45 + random.nextInt(55), 255, 224, 137)
                    canvas.drawCircle(x, y, radius * 0.75f, paint)
                }
                "autumn" -> {
                    paint.color = Color.argb(80 + random.nextInt(75), 232, 128 + index % 55, 68)
                    canvas.save()
                    canvas.rotate((index * 29L + frame % 23L).toFloat(), x, y)
                    canvas.drawOval(RectF(x - radius * 1.8f, y - radius, x + radius * 1.8f, y + radius), paint)
                    canvas.restore()
                }
                else -> {
                    paint.color = Color.argb(50 + random.nextInt(55), 222, 241, 255)
    ...
    private fun drawHeatScene(canvas: Canvas, frame: Long, paint: Paint) {
        ...
        paint.shader = RadialGradient(
            ...
            28f,
            215f,
            Color.argb(66, 255, 189, 88),
            Color.TRANSPARENT,
            Shader.TileMode.CLAMP,
        )
        canvas.drawCircle(390f, 28f, 215f, paint)
        paint.shader = null
        val random = Random(1700L + frame)
        repeat(10) {
            paint.color = Color.argb(35 + random.nextInt(40), 255, 216, 137)
            canvas.drawCircle(
                20f + random.nextFloat() * 440f,
                30f + random.nextFloat() * 150f,
