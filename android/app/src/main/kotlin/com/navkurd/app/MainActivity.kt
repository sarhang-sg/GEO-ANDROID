...
class MainActivity : FlutterActivity(), EventChannel.StreamHandler {
        ...
        try {
            when (call.method) {
                ...
                "showNotificationOnce" -> {
                    result.success(
                        showNotificationOnce(
                            call.argument<String>("key") ?: "general",
                            call.argument<String>("title") ?: getString(R.string.app_name),
                            call.argument<String>("body") ?: "",
                        ),
                    )
    ...
    private fun showNotificationOnce(key: String, title: String, body: String): Boolean {
        val safeKey = key.lowercase(Locale.ROOT)
            .replace(Regex("[^a-z0-9._-]"), "-")
            .take(80)
        val preferences = getSharedPreferences(NOTIFICATION_PREFS, MODE_PRIVATE)
        if (preferences.getBoolean(safeKey, false)) return false
        if (!showNotification(title, body)) return false
        preferences.edit().putBoolean(safeKey, true).apply()
        return true
    }
