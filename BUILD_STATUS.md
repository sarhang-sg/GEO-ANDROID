...
- Locked Android satellite rendering to hardware-accelerated hybrid composition,
  disabled algorithmic darkening through the supported WebView API, rejected the
  deprecated `forceDark` setting and enabled offscreen raster preparation so
  satellite tiles do not turn black in the APK.
