...
readonly artifact_name="NAV-KURD-9.0.0-signed-release"
temp_dir=""
backup_stash=""

cleanup() {
  if [[ -n "$temp_dir" && "$temp_dir" == "$HOME"/.geo-android-v9.* ]]; then
    rm -rf -- "$temp_dir"
  fi
}
trap cleanup EXIT
...
if gh repo view "$repo_name" >/dev/null 2>&1; then
  ...
  retry gh repo create "$repo_name" \
    --private \
    --description "NAV KURD 9.0.0 Flutter Android"
fi
if gh api "repos/$repo_name/branches/main" >/dev/null 2>&1; then
  repo_has_main=true
fi

echo "2/12 — Verifying the final Android source payload..."
source_zip="$(find "$download_dir" -maxdepth 2 -type f \
  -name 'NAV-KURD-v9.0.0-ANDROID.zip' \
  -printf '%T@ %p\n' 2>/dev/null |
  sort -nr | head -n 1 | cut -d' ' -f2-)"
[[ -n "$source_zip" && -s "$source_zip" ]] ||
  die "NAV-KURD-v9.0.0-ANDROID.zip was not found in Download."
...
temp_dir="$(mktemp -d "$HOME/.geo-android-v9.XXXXXX")"
...
if [[ -n "$(git -C "$project_dir" status --porcelain --untracked-files=all)" ]]; then
  backup_label="NAV-KURD-9.0.0-SAFE-BACKUP-$(date -u +%Y%m%d-%H%M%S)"
...
if [[ "$repo_has_main" == true ]]; then
  ...
  branch_name="codex/nav-kurd-v9-$(date -u +%Y%m%d-%H%M%S)-$$"
...
if git -C "$project_dir" diff --cached --quiet && [[ "$repo_has_main" == true ]]; then
  echo "The repository already contains NAV KURD 9.0.0."
  git -C "$project_dir" switch main
  ...
  git -C "$project_dir" commit -m "Release NAV KURD Android 9.0.0"
  ...
  if [[ "$repo_has_main" == true ]]; then
    ...
    pull_request_url="$(retry gh pr create \
      ...
      --head "$branch_name" \
      --draft \
      --title "NAV KURD Android 9.0.0" \
      --body "Android 9.0.0: native image picker and share bridge, stable search/GPS focus, compact premium UI, multilingual widget synchronization, diagnostics fixes and verified release signing.")"
...
if [[ "$conclusion" != "success" ]]; then
  error_log="$download_dir/GEO-ANDROID-V9-ERROR-$run_id.txt"
  failed_log="$temp_dir/GEO-ANDROID-V9-FAILED-$run_id.txt"
...
release_dir="$download_dir/GEO-ANDROID-V9-RELEASE-$run_id"
mkdir -p "$release_dir"
...
universal_apk="$release_dir/NAV-KURD-9.0.0.apk"
[[ -s "$universal_apk" ]] || die "Universal NAV-KURD-9.0.0.apk was not found."
...
install_apk="$download_dir/NAV-KURD-9.0.0-$run_id.apk"
...
echo "9/12 — Preparing the signed direct-download update on current Web main..."
...
grep -F 'ATLAS_TEXT_LIMITS.name, "kurdish"' \
  ...
  die "Run the final Web fixes first; place-language validation is not current."
grep -F 'rgba(37, 0, 43, .95)' \
  "$web_repo_dir/src/styles/components/loader.css" >/dev/null ||
  die "Run the final Web fixes first; the approved loader overlay is not current."
[[ -s "$web_repo_dir/public/fonts/DroidLogo-Bold.ttf" ]] ||
  die "Run the final Web fixes first; DroidLogo-Bold.ttf is missing."

web_branch_name="codex/nav-kurd-android-download-v9-$(date -u +%Y%m%d-%H%M%S)-$$"
git -C "$web_repo_dir" switch -c "$web_branch_name"
mkdir -p "$web_repo_dir/public/downloads"
cp -f "$universal_apk" "$web_repo_dir/public/downloads/NAV-KURD-9.0.0.apk"
...
if git -C "$web_repo_dir" diff --cached --quiet; then
  echo "The Web repository already contains this exact signed APK and metadata."
  git -C "$web_repo_dir" switch main
  ...
  git -C "$web_repo_dir" commit -m \
    "Publish signed NAV KURD Android 9.0.0 download"
  web_head_sha="$(git -C "$web_repo_dir" rev-parse HEAD)"
  ...
  web_pr_url="$(retry gh pr create \
    ...
    --draft \
    --title "Publish signed NAV KURD Android 9.0.0" \
    --body "Adds only the certificate-verified NAV KURD 9.0.0 APK and generated download metadata to the already-complete Web main. No Web UI or runtime source is replaced.")"
  ...
  if [[ "$web_conclusion" != "success" ]]; then
    web_error_log="$download_dir/GEO-WEB-V9-ERROR-$web_run_id.txt"
    {
...
echo "Vercel will deploy the signed direct-download update automatically."
